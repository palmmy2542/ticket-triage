...
summarises this; the numbers come from `pnpm eval --repeat 2` against `gpt-4.1-mini`, and the
final report is committed at `results/baseline-gpt-4.1-mini.json`.
...
- **Groundedness of the reply text.** The guard forces a relevant article to exist, not that
  it was read correctly. A model could still cite release 4.2 while the account is on 4.1.
  That needs an LLM-as-judge over draft-versus-evidence.
